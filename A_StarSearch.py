...
def tiled_Astar(
    ...
    Node            = namedtuple("Node", ['f_score', 'g_score', 'tile'])
    openSet         = [Node(heuristic(origin_tile, target_tile) , 0, origin_tile)]
    closedSet       = set()
    ...
    while openSet:
        ...
        for neighbour in check_neighbour(node=current.tile):
            ...
            f_score = g_score + heuristic(neighbour, target_tile)
            ...
            if neighbour not in closedSet and neighbour not in [node.tile for node in openSet]:
                neighbour_node = Node(f_score, g_score, neighbour)
